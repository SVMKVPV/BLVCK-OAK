...
import { applyPaidSession, audToCents, createQuote, CUSTOM_CHECKOUT_VERSION, markQuotePaymentReview, rotatePublicToken, nextPayment, publicQuote, settleQuotePayment, validateQuoteInput } from '../netlify/lib/quotes.mjs';
...
test('rotating a private client token revokes the previous Black Oak link without changing payment records', () => {
  const quote = approvedQuote();
  const rotated = rotatePublicToken(quote, 'owner-account');
  assert.notEqual(rotated.publicToken, quote.publicToken);
  assert.match(rotated.publicToken, /^[A-Za-z0-9_-]{43}$/);
  assert.equal(rotated.clientLinkRotatedBy, 'owner-account');
  assert.equal(rotated.payments.length, quote.payments.length);
  assert.equal(rotated.status, quote.status);
});
