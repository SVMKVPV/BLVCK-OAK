...
const checkoutReferralField = document.querySelector('[data-checkout-referral-field]');
const checkoutBillingNote = document.querySelector('[data-checkout-billing-note]');
const summaryPriceLabel = document.querySelector('[data-summary-price-label]');
...
function setCheckoutButtonState(loading = false) {
  if (!checkoutButton || !selectedPackage) return;
  checkoutButton.disabled = loading;
  checkoutButton.firstChild.textContent = loading
    ? 'Opening checkout… '
    : selectedPackage.billing === 'monthly'
      ? 'Start monthly plan '
      : 'Continue to secure checkout ';
  checkoutButton.querySelector('span').textContent = loading ? '•' : '↗';
}
...
checkoutButton?.addEventListener('click', async () => {
  ...
  const referralCode = selectedPackage.referralEligible === false ? '' : getActiveReferralCode();
...
if (paymentNotice && (paymentStatus === 'success' || paymentStatus === 'cancelled')) {
  ...
  const purchasedPackage = packageCatalog[pageParameters.get('package') || ''];
  const subscriptionStarted = success && purchasedPackage?.billing === 'monthly';
  paymentNotice.classList.add(paymentStatus);
  paymentNotice.querySelector('[data-payment-icon]').textContent = success ? '✓' : '↩';
  paymentNotice.querySelector('[data-payment-title]').textContent = subscriptionStarted
  ...
  paymentNotice.querySelector('[data-payment-message]').textContent = success
    ? subscriptionStarted
      ? ' Your Link in Bio subscription is active—we’ll be in touch with onboarding details.'
...
function renderReferralLinks(code) {
  ...
  Object.values(packageCatalog).filter((packageItem) => packageItem.referralEligible !== false).forEach((packageItem) => {
